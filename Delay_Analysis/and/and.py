...
fp3 = open("output_and.txt", 'w')
fp3.close()

# this is the command this helps to choose which file to run
command = "ngspice destination_and.ckt"
...
for j in range(0, 8):  # j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    if j < 4:
        s1 = r"a" + str(j)  # for input sequence 0000 1111 output = 1111 s1 indicates string1
    else:
        s1 = "b" + str(j - 4)  # for input sequence 1111 0000 output = 1111

    for i in range(0, 4):  # i is for output sequence

        s2 = "and" + str(i)  # s2 indicates string 2.

        # fp1 is src file since you cant modify it you open it in read mode
        if j < 4:
            fp1 = open("and1delay.ckt", 'r')  # for input sequence 1
        else:
            fp1 = open("and2delay.ckt", 'r')  # for input sequence 2

        fp2 = open("destination_and.ckt", 'w')  # destination file for running
        fp3 = open("output_and.txt", 'a')  # final output file
        mode1 = "RISE"
        mode2 = "FALL"
...
crit_path = max(path_delays, key=path_delays.get)
max_delay = path_delays[crit_path]
