...
fp3 = open("output_add.txt", 'w')
fp3.close()

# this is the command this helps to choose which file to run
command = "ngspice destination_add.ckt"
...
for j in range(0, 8):  # j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    ...
    for i in range(0, 4):  # i is for output sequence
        ...
        s2 = "sum" + str(i)  # s2 indicates string 2.

        # fp1 is src file since you can't modify it you open it in read mode
        if j < 4:
            fp1 = open("add1delay.ckt", 'r')  # for input sequence 1
        else:
            fp1 = open("add2delay.ckt", 'r')  # for input sequence 2

        fp2 = open("destination_add.ckt", 'w')  # destination file for running
        fp3 = open("output_add.txt", 'a')  # final output file
        mode1 = "RISE"
        mode2 = "FALL"
